...
def ruleset_definition(request, ruleset_name):
    ...
    rule_engine_response = request.post(url=url, description)

    # create response_body
    try:
        response_data = RESP_CODES.get(int(rule_engine_response['outcome']))
    except:
        response_data = rule_engine_response
    resp = {
...
def ruleset_facts(request, ruleset_name):
    ...
    response_data = request.post(url=url, query_data)
    resp = {
        "ruleset_name": ruleset_name,
        "type": "facts",
        "timestamp": datetime.datetime.now().isoformat(),
